...
class Virus():
    def __init__(self, factors):
        ...
        # avoiding x and y axes
        self.axes.set_ylim(0, 1)

        # create annotations
        self.day_text = self.axes.annotate(
            "Day 0", xy=[np.pi / 2, 1], ha="center", va="bottom")
        # defining the position of each text
        self.infected_text = self.axes.annotate(
            "Infected: 0", xy=[3 * np.pi / 2, 1], ha="center",
            va="top", color=RED)
        self.deaths_text = self.axes.annotate(
            "\nDeaths: 0", xy=[3 * np.pi / 2, 1], ha="center",
            va="top", color=BLACK)
        self.recovered_text = self.axes.annotate(
            "\n\nRecovered: 0", xy=[3 * np.pi / 2, 1], ha="center",
            va="top", color=GREEN)
        ...
        # to track current and total number of infected ones
        self.num_recovered = 0
        self.num_deaths = 0
        self.r0 = factors["r0"]
        # storing the values of dict_covid19_FACTORS
        self.percent_mild = factors["percent_mild"]
        self.percent_sev = factors["percent_sev"]
        self.fatality_rate = factors["fatality_rate"]
        self.serial_interval = factors["serial_interval"]
        # gives the avg time int bn succ_cases
        ...
        self.mild = {i: {"thetas": [], "rs": []} for i in range(
            self.mild_fast, 365)}
        # creating a dict to track mild case
        self.sev = {
            "recov": {i: {"thetas": [], "rs": []} for i in range(
                self.sev_fast, 365)},
            "death": {i: {"thetas": [], "rs": []} for i in range(
                self.death_fast, 365)}
            # classifying the sev case
        }

        self.exposed_before = 0
        self.exposed_after = 1
        # these variables imposes extension in the virus spread
    ...
    def initial_population(self):
        ...
        # keeps tracking everyone in the population
        ...
        if self.day % self.serial_interval == 0 and self.exposed_before < 4500:
            ...
            self.new_infected_indices = list(
                np.random.choice(
                    ...
                    replace=False))
            # creating a list for new infected ones
    ...
    def assign_symptoms(self):
        ...
        # calculating values from the dict info
        ...
        if remaining_indices:
            self.sev_indices = np.random.choice(
                remaining_indices, num_sev_recov, replace=False)
            self.death_indices = [
                i for i in remaining_indices if i not in self.sev_indices]
        ...
        for mild in self.mild_indices:
            ...
            mild_r = self.rs[mild]
            # retreiving data to locate coordinate
            self.mild[recov_day]["thetas"].append(mild_theta)
